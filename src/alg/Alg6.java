 ...
 * 分析：首先是两遍的循环，第一遍记录字符出现的次数，第二遍找到最先出现一次字符。
 *  
 * 2、对称子字符串的最大长度
 * 输入一个字符串，输出该字符串中对称的子字符串的最大长度。比如输入字符串“google”，由于该字符串里最长的对称子字符串是“goog”，因此输出4。
 * 提示： 求最大的回文字符串 Alg4种有代码
 * 
 * 3、编程判断俩个链表是否相交
 * 给出俩个单向链表的头指针，比如h1，h2，判断这俩个链表是否相交。为了简化问题，我们假设俩个链表均不带环。
 * 提示：如相交则后面的全部一样，只需要判断最后的一个数值即可 
 * 问题扩展：
 * •	如果链表可能有环列?  声明两个指针，一个以一的速度向后，一个以二的速度向后
 * •	如果需要求出俩个链表相交的第一个节点列? 从后面向前推。
 * 
 * 4、逆序输出链表 
 ...
 * 提示：把后面的数据一次复制到前面来，删除后面的这个节点即可（为尾节点则没有办法）
 * 
 * 6、找出链表的第一个公共结点 
 * 两个单向链表，找出它们的第一个公共结点。
 * 提示：类似3
 ...
 * 提示：可以采用签名的方法，首先确认aeiou的签名，然后遍历 they are students 去除不合适的字符
 * 
 * 8、字符串的匹配
 * 
